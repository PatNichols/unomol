#ifndef UNOMOL_UHF_HPP
#define UNOMOL_UHF_HPP
#include <cstdio>
#include <cstdlib>
#include <cmath>
#include <iostream>
...
class UnRestrictedHartreeFock {
  public:
    ...
    void formCmatrix(double* c) noexcept {
        copy_trans(no,Wmat);
        for (int i=0; i<no; ++i) {
    ...
    constexpr void formPmatrix(double* p,double *c,int noc) noexcept {
        int ij=0;
        for (int i=0; i<no; ++i) {
    ...
    void FiniteFieldAnalysis() noexcept {
    ...
    void findPolarizationPotential() noexcept {
        double px,py,pz;
