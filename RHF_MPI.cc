...
{
public:
    RestrictedHartreeFockMPI(Basis* b,const TwoElectronInts* x);
...
RestrictedHartreeFockMPI::RestrictedHartreeFockMPI(Basis* b,
...
RestrictedHartreeFockMPI::~RestrictedHartreeFockMPI()
{
    if (!rank)
    {
        delete [] Wrka;
...
inline void RestrictedHartreeFockMPI::update()
{
    register int i;
...
inline void RestrictedHartreeFockMPI::dpm_update(const TwoElectronInts& xints)
{
    register int i;
    MPI_Barrier(MPI_COMM_WORLD);
    MPI_Bcast(Pmat,no2,MPI_DOUBLE,0,MPI_COMM_WORLD);
...
inline void RestrictedHartreeFockMPI::findEnergy()
{
    if (!rank)
    {
...
inline void RestrictedHartreeFockMPI::formCmatrix(double* c)
{
    copy_trans(no,Wmat);
...
inline void RestrictedHartreeFockMPI::formPmatrix(double* p,double *c,int noc)
{
    register int ij=0;
    for (register int i=0;i<no;++i)
    {
...
inline void RestrictedHartreeFockMPI::PmatrixGuess()
{
...
inline void RestrictedHartreeFockMPI::formXmatrix()
{
...
inline void RestrictedHartreeFockMPI::FiniteFieldAnalysis()
{
...
inline double RestrictedHartreeFockMPI::nuclear_repulsion_energy(int ncen,
        const Center* center)
{
    register double sum=0.0;
    double q1,q2,r12;
...
inline void RestrictedHartreeFockMPI::findPolarizationPotential()
{
    double px,py,pz;
...
inline bool RestrictedHartreeFockMPI::is_converged()
{
    switch (cflag)
    {
    case 0:
...
inline void RestrictedHartreeFockMPI::final_output(double init_energy)
{
...
inline void RestrictedHartreeFockMPI::OintsOutput()
{
...
inline void
RestrictedHartreeFockMPI::scf_converger()
{
    register int i;
...
inline void RestrictedHartreeFockMPI::PopulationAnalysis(FILE *out)
{
    const Center *center=basis.center_ptr();
