#ifndef _SymmPack_CC_
#define _SymmPack_CC_
#include <iostream>
#include <cstdio>
#include <cstdlib>
#include <cmath>
...
inline double TraceSymmPackProduct(
    double a[],double b[],int n)
{
    register double sum=0.0;
    register int ij=0;
    for (register int i=0;i<n;++ij,++i) {
        for (register int j=0;j<i;++ij,++j) {
            sum+=a[ij]*b[ij];
        }
        sum+=0.5*a[ij]*b[ij];
    }
...
inline double SymmPackDiffNorm(
    double a[],double b[],int n)
{
    register double term=0.0;
    register double sum=0.0;
    register int ij=0;
...
inline void reduce(const int n,double* __restrict__ a,double* __restrict__ d,
...
inline int tql2(const int n,double* __restrict__ d,double* __restrict__ e,
...
*/
void
sp_trans (int n, double *a, double *z, double *tmp)
{
    register int i, j, k;
    int ij, ik, ii, istr;
